...
app.controller("ctrlAdmin", function ($scope, $http) {
  ...
  $scope.imageChangeAccount = function (files) {
    ...
    $http
      .post(`http://localhost:8080/rest/upload/imageAccount`, data, {
      ...
      .catch((error) => {
        Swal.fire("Error", "Bạn đã gặp lỗi khi upload ảnh :(", "error");
      });
  };
  ...
  $scope.create = function () {
    ...
    $http.post(url, data).then(
      function (res) {
        Swal.fire(
          "Good job!",
          "Đã thêm thành công Account: " + data.name,
          "success"
        );
        ...
        $http.post(`${hostAccount}/accountRole`, itemrole).then(
          function (resp) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http
          ...
          .catch((error) =>
            Swal.fire("Error", "Xin lỗi, đang được sử dụng :)", "error")
          );
        // For more information about handling dismissals please visit
        // https://sweetalert2.github.io/#handling-dismissals
      } else if (result.dismiss === Swal.DismissReason.cancel) {
        Swal.fire("Cancelled", "Lần sau cẩn thận hơn nhé :)", "error");
      }
    });
  };
  //---------------------------------------------------------------
